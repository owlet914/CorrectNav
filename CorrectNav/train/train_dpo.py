...
from CorrectNav.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX
from torch.utils.data import Dataset
from CorrectNav.train.llava_trainer import LLaVADPOTrainer
#from data_processing.utils import load_jsonl, load_json
from CorrectNav import conversation as conversation_lib
...
from CorrectNav.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token
        ...
        with open(file_path, 'r') as f:
            for line in f:
                try:
                    ...
                    # You might want to handle invalid lines differently (e.g., skip them, log them)
    except FileNotFoundError:
        print(f"Error: File not found at {file_path}")
        return []
    except Exception as e:
...
class ModelArguments:
    ...
    model_class_name: Optional[str] = field(default=None, metadata={"help": "Used to init model class, format is XXXXForCausalLM. e.g. currently XXXX is chosen from CorrectNavLlama, LlavaMixtral, LlavaMistral, Llama"})
