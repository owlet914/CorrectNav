...
if __name__ == "__main__":
    ...
    device = "cuda"
    device_map = "auto"      
    log_file = os.path.join('/LingXi/LLaVA-NeXT/vln_eval', pretrained.split('/')[-2] + f"_ckpt-{ckpt_chosen}.log")
    logging.basicConfig(filename=log_file, level=logging.INFO, 
                        format='%(asctime)s - %(levelname)s - %(message)s')

    habitat_config = rxr_w_config(stage='val_unseen',img_size=1024)
    ...
    while True:
        ...
        while True:
            ...
            if step_idx==0:
                ...
                rgb=cv2.resize(obs['rgb'], (384,384))
                rgb_input_traj.append(rgb)
            video, frame_time, video_time = process_images_as_video(rgb_input_traj, original_fps = 1, max_frames_num = 16, target_fps=1, force_sample=False)
        ...
        ndtw=metrics['ndtw'] if NDTW else 0
        print(instr, success, spl, dtg)
        
        file=locked_load_json(file_path)
        for i, item in enumerate(file):
            if instr == list(item.keys())[0]:
                file[i][instr] = (success, spl, dtg,ndtw)
                break  
        locked_dump_json(file,file_path)   
        success_rates, spl_results, dtg_results,ndtw_results = [], [], [],[]
        for item in file:
            ...
            if all(math.isfinite(x) for x in result):
                ...
                if NDTW:
                    ndtw_results.append(result[3])
                else:
                    ndtw_results.append(0)
                

        
        avg_sr, avg_spl, avg_dtg ,ndtw_avg= np.mean(success_rates), np.mean(spl_results), np.mean(dtg_results),np.mean(ndtw_results)
        
        num_eval = len(success_rates)
        eval_result = 'Success:%.3f,SPL:%.2f,Nav Error:%.2f,ndtw:%.2f'%(avg_sr, avg_spl, avg_dtg,ndtw_avg)
