...
`json_path` is the full path to each generated training JSON. The trajectory-to-instruction JSON files are generated by `data/traj2instruct.py`, and the correction JSON files are generated by `data/train_json.py`. `sampling_strategy` controls how many samples are loaded from each JSON and supports `all`, `first:N`, `end:N`, and `random:N`.
