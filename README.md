...
### 3.5 General Visual Understanding Data Preparation
...
Write the annotation JSON or JSONL paths from these subsets into `scripts/train_mixed.yaml` together with the GT VLN-CE JSON files generated in Section 3.4.

### 3.6 Correction Data Collection
...
### 3.7 Correction Image Data to Video
...
### 4.1 Mixed Dataset YAML

Write the generated training JSON files into `scripts/train_mixed.yaml` before starting training.

```yaml
datasets:
  - json_path: /path/to/vlnce_r2r_t2i.json
    sampling_strategy: all
  - json_path: /path/to/vlnce_rxr_t2i.json
    sampling_strategy: all
  - json_path: /path/to/LLaVA-Video-178K/0_30_s_academic_v0_1.json
    sampling_strategy: all
  - json_path: /path/to/LLaVA-Video-178K/0_30_s_activitynetqa.json
    sampling_strategy: all
  - json_path: /path/to/LLaVA-Video-178K/0_30_s_nextqa.json
    sampling_strategy: all
  - json_path: /path/to/LLaVA-Video-178K/0_30_s_perceptiontest.json
    sampling_strategy: all
  - json_path: /path/to/vlnce_r2r_video_fly.json
    sampling_strategy: all
  - json_path: /path/to/vlnce_rxr_video_fly.json
    sampling_strategy: all

```

`json_path` is the full path to each training JSON or JSONL. The trajectory-to-instruction JSON files are generated by `data/traj2instruct.py`, the general visual understanding data comes from the selected LLaVA-Video-178K subsets, and the correction JSON files are generated by `data/train_json.py`. `sampling_strategy` controls how many samples are loaded from each file and supports `all`, `first:N`, `end:N`, and `random:N`.

### 4.2 Training Script Configuration
